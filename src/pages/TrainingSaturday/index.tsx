...
export function TrainingSaturday() {
  const { trainingWeeks, setTrainingWeeks } = useContext(SaturdayContext);
  ...
  return (
    <ContainerWeek>
      {trainingWeeks.length === 0 ? (
        <>
          <Title>O que você vai treinar Sábado?</Title>
          <ContentWorkouts>
        ...
        <div>
          {trainingWeeks.some((training) => !training.rest) && (
            <Title2>Na Sábado você vai treinar:</Title2>
          )}
